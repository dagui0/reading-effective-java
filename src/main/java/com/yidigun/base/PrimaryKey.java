...
/**
 * {@link DomainObject}의 Primary Key의 값을 담는 DTO(Data transfer object) 인터페이스.
 ...
 * 그 이유는 보통 PK에 사용되는 integer, long, string 등의 기본 타입은 범용 타입이라서
 * 어느 테이블의 키값인지 확인할 수 없기 때문이다.
 * <p>
 * 키 전용 래핑 클래스를 만들게 되면 타입 안전성이 향상되고 버그 발생 가능성이 많이 줄어들 수 있다.
 *
 * <h3>필요에 따라 {@link Comparable}이나, {@link CharSequence} 등을 추가로 구현할 수 있다.</h3>
 *
 * 정렬에 사용될 수 있게 하려면 {@link Comparable}를 추가로 구현할 수 있다.
 *
 * <code>
 ...
 * </code>
 *
 * 만약 키의 정의가 고정 자리수별로 특정한 의미를 나타내는 경우(Structured Key, Semantic Key)
 * {@link CharSequence}를 구현하여 편의기능을 추가할 수 있다.
 *
 * <code>
 ...
 * </code>
 *
 * <h3>Key 타입을 독립 클래스로 만들 것인지 중첩 클래스로 만들 것인지</h3>
 *
 ...
 * </code>
 *
 * @see DomainObject 도메인 객체 인터페이스
 * @see MemberKey 독립 클래스방식 구현 예시
 */
